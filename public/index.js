function fetchMoviesDetail() {
    ...
    const targetDt = targetDateInput.value 
        ? `${targetDate.getFullYear()}${('0' + (targetDate.getMonth() + 1)).slice(-2)}${('0' + targetDate.getDate()).slice(-2)}`
        : '20230314'; // 기본값으로 20230314 설정

    fetch(`/movies?targetDt=${targetDt}`)
    .then(response => response.json())
    .then((json) => {
        const ul = document.querySelector("#v-pills-home");
        ul.innerHTML = '';  // 기존 내용을 지웁니다
        if (json.result && Array.isArray(json.result)) {
            for (let i = 0; i < json.result.length; i++) {
                ul.innerHTML += `<li>${json.result[i].movieNm}</li>`;
            }
        } else {
